...
function isSinglesMode() {
  return eventData?.gameMode === "singles";
}
...
function competitorMembers(team) {
  ...
  return team.players?.filter(Boolean).join(" & ") || "Waiting for players";
}

function competitorReady(team) {
  if (!team) return false;
  if (isSinglesMode()) return Boolean(team.players?.[0]);
  return Boolean(team.name && team.players?.length >= 2 && team.players.every(Boolean));
}

function updateGameModeControls() {
  const mode = $("#gameMode").value;
  const countSelect = $("#teamCount");
  const current = Number(countSelect.value) || (mode === "singles" ? 4 : 4);
  countSelect.innerHTML = "";

  const max = mode === "singles" ? 4 : 12;
...
function renderAll() {
  ...
  $("#metricMode").textContent = `${isSinglesMode() ? "Singles" : "Ambrose"} • ${eventData.status}`;
...
async function startTournament() {
  ...
  await updateDoc(doc(db, "events", eventCode), {
    status: "playing",
    story: [...(eventData.story || []), `Tournament started with ${teams.length} ${isSinglesMode() ? "players" : "teams"}.`]
  });
}
