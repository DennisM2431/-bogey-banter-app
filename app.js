...
async function renderQrCodes() {
  ...
  for (const team of teams) {
    ...
    card.innerHTML = `<canvas class="qr-target" aria-label="Team ${team.slot} join QR code"></canvas><strong>Team ${team.slot}</strong><small>${escapeHtml(team.name || "Unclaimed")}</small>`;
    grid.appendChild(card);
    const qrCanvas = card.querySelector(".qr-target");
    try {
      ...
      qrCanvas.replaceWith(Object.assign(document.createElement("a"), {
        href: link,
        textContent: "Open Team " + team.slot + " join link"
      }));
    }
  }
}
