...
function activeSlot() {
  if (eventData?.playStyle === "shared") return Number(eventData.currentTurnSlot || 1);
  return teamSlot;
}
...
function renderAll() {
  ...
  if (sharedBanner) {
    ...
    if (isSharedDevice() && current) {
      sharedBanner.innerHTML = `<span>NOW PLAYING</span><strong>${escapeHtml(competitorLabel(current))}</strong><small>Hole ${eventData.hole} • Pass the phone when prompted</small>`;
    }
  }
...
function renderDraw() {
  ...
  $("#drawCard")?.addEventListener("click", async () => {
    ...
    if (isSharedDevice()) showScreen("scoreScreen");
  });
}
...
function renderScoring() {
  ...
  if (isSharedDevice()) {
    if (!team) {
      box.innerHTML = `<div class="panel">No current player found.</div>`;
      return;
    }
    const drawn = team.drawnHole === eventData.hole;
    const alreadySubmitted = team.scoreHole === eventData.hole;
    const next = teams.find((entry) => entry.slot === sharedNextSlot(team.slot));
    box.innerHTML = `<div class="panel shared-score-panel">
      <div class="turn-kicker">PASS THE PHONE TO ${escapeHtml(competitorLabel(next))}</div>
      <h2>Enter ${escapeHtml(competitorLabel(team))}'s score</h2>
      ${drawn ? `<div class="callout"><strong>${escapeHtml(team.card[2])} ${escapeHtml(team.card[0])}</strong><br>${escapeHtml(team.card[3])}</div>` : `<div class="callout">Draw ${escapeHtml(competitorLabel(team))}'s card first.</div>`}
      <label>Total strokes
        <input id="sharedScore" type="number" min="1" max="30" value="${alreadySubmitted ? team.score ?? "" : ""}" ${alreadySubmitted || !drawn ? "disabled" : ""}>
      </label>
      <button id="submitSharedScore" class="${alreadySubmitted ? "score-submitted-lock" : "primary"}" ${alreadySubmitted || !drawn ? "disabled" : ""}>
        ${alreadySubmitted ? "Score confirmed ✓" : "Confirm score"}
      </button>
    </div>`;

    if (!alreadySubmitted && drawn) {
      $("#submitSharedScore").addEventListener("click", async () => {
        ...
        const isLast = team.slot === teams.length;
        if (isLast) {
          await updateDoc(doc(db, "events", eventCode), {
            currentTurnSlot: 1,
          ...
          await updateDoc(doc(db, "events", eventCode), {
            currentTurnSlot: sharedNextSlot(team.slot)
          });
