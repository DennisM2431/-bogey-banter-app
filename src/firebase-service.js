...
import {
  getFirestore,doc,setDoc,addDoc,getDoc,updateDoc,onSnapshot,collection,writeBatch,serverTimestamp
...
export async function createEvent(settings,user){
  ...
  for(let slot=1;slot<=settings.count;slot++){
    ...
    batch.set(doc(db,"events",code,"teams",String(slot)),{
      ...
      drawnHole:0,card:null,bonusCards:[],scoreHole:0,score:null,acceptedHole:0,
      cumulativeScore:0,categoryCounts:{},bestHole:null,worstHole:null,readyBreakHole:0,votesLocked:false,votes:{}
    });
  }
  await batch.commit();
  return code;
...
export async function submitFeedback(payload){
  ...
  return addDoc(collection(db,"feedback"),{...payload,userUid:user.uid,createdAt:serverTimestamp()});
}
export function connectionInfo(){return {authenticated:Boolean(auth.currentUser),projectId:firebaseConfig.projectId};}
