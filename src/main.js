...
function activeCards(team){
  if(!team||team.drawnHole!==state.event?.hole)return [];
  return [team.card,...(team.bonusCards||[])].filter(Boolean);
}
function isChaosCombo(card){return card?.[0]==="Chaos Combo"}
function isDoubleTrouble(card){return card?.[0]==="Double Trouble"}
function bonusRequired(team){return isChaosCombo(team?.card)?2:isDoubleTrouble(team?.card)?1:0}
function bonusComplete(team){return (team?.bonusCards||[]).length>=bonusRequired(team)}
function allDrawn(){return state.teams.length&&state.teams.every(t=>t.drawnHole===state.event.hole&&t.card&&bonusComplete(t))}
function cardImage(card){return `./assets/cards/${escapeHtml(card?.[4]||"card-001")}.jpg`}
function votingTeam(){
  if(isShared())return state.teams.find(t=>t.slot===Number(state.event.currentVoteSlot||1));
  return state.teams.find(t=>t.slot===state.teamSlot);
}
function allVotesSubmitted(){return state.teams.length>0&&state.teams.every(t=>t.votesLocked)}
function allPlayers(){
  return state.teams.flatMap(t=>(t.players||[]).filter(Boolean).map((name,index)=>({id:`${t.slot}:${index}`,name,teamSlot:t.slot,teamName:label(t)})));
}
...
function chrome(content,nav=true){
  ...
  const artKey=state.mode==="play"?(state.screen||"lobby"):state.mode;
  return `<main class="shell screen-art screen-art-${artKey}">
    <header class="topbar">
      ...
      <div class="topbar-actions">
        ...
        ${isDemo()?'<span class="demo-ribbon">DEMO MODE</span>':'<span class="version">v7.0 RC1.2</span>'}
      </div>
    </header>
...
function registration(){
  ...
  const selectedByOthers=new Map(state.teams.filter(t=>t.slot!==team.slot&&t.name).map(t=>[t.name,label(t)]));
  const names=TEAM_NAMES.map(n=>{
    const used=selectedByOthers.has(n);
    return `<option value="${escapeHtml(n)}" ${team.name===n?"selected":""} ${used?"disabled":""}>${escapeHtml(n)}${used?` — Already selected`:""}</option>`;
  }).join("");
  return `<section class="card"><div class="kicker">YOUR ENTRY</div><h2>Register ${state.event.gameMode==="singles"?"player":"team"}</h2>
    <div class="form-grid">
      ${state.event.gameMode!=="singles"?`<label class="field wide"><span>Team name</span><select id="regName"><option value="">Choose a unique team name</option>${names}</select></label>`:""}
...
function draw(){
  if(state.event.status==="lobby")return chrome(`<section class="card"><h2>Waiting for the host to start</h2></section>`);
...
function sharedDraw(){
  if(state.event.sharedRevealSlot){
    ...
    const last=allDrawn();
    if(bonusRequired(t)&&!bonusComplete(t)){
      return `<section class="card"><div class="kicker">${escapeHtml(label(t))} ACTIVATED EXTRA CHAOS</div>${cardsMarkup(t)}
        <button id="drawBonusCards" class="primary" style="margin-top:12px">Draw ${bonusRequired(t)} Bonus Card${bonusRequired(t)>1?"s":""}</button></section>`;
    }
    return `<section class="card"><div class="kicker">${escapeHtml(label(t))} DREW</div>${cardsMarkup(t)}
  ...
  return `<section class="card">${drawCardView(ownTeam(),true)}</section>`;
}

function drawCardView(team,shared){
  const drawn=team.drawnHole===state.event.hole&&team.card;
  if(!drawn){
    return `<div class="draw-deck"><div class="deck-back">BB</div><div class="kicker">${shared?`${escapeHtml(label(team))}'S TURN`:"ONE DRAW PER HOLE"}</div><h2>Draw Your Chaos Card</h2><p>${shared?"Everyone draws before scoring starts.":"Your paired scorer sees it immediately."}</p></div><button id="drawButton" class="primary" style="margin-top:12px">Draw Card</button>`;
  }
  return `${cardsMarkup(team)}
    ${bonusRequired(team)&&!bonusComplete(team)?`<button id="drawBonusCards" class="primary chaos-bonus-button">🎴 Draw ${bonusRequired(team)} Bonus Card${bonusRequired(team)>1?"s":""}</button>`:`<button class="secondary" disabled style="margin-top:12px">Card draw complete ✓</button>`}`;
}
function cardClass(card){return CARD_CATEGORIES[card?.[1]]?.className||"challenge"}
function cardMarkup(card,owner=""){
  return `<article class="chaos-card chaos-${cardClass(card)}">
    <img class="chaos-card-image" src="${cardImage(card)}" alt="${escapeHtml(card[0])}">
    <div class="chaos-card-overlay">
      <div class="rarity">${escapeHtml(card[1])} • CARD ${String(card[6]||0).padStart(3,"0")} • HOLE ONLY</div>
      <h2>${escapeHtml(card[2])} ${escapeHtml(card[0])}</h2>
      <p class="card-subtitle">${escapeHtml(card[5]||"")}</p>
      <div class="rule-panel"><p>${escapeHtml(card[3])}</p></div>
      <div class="card-footer"><span>${escapeHtml(owner)}</span><span>Hole ${state.event.hole}</span></div>
    </div>
  </article>`;
}
function cardsMarkup(team){
  const cards=activeCards(team);
  return `<div class="active-card-stack">${cards.map((c,i)=>`${i?'<div class="combo-plus">+</div>':""}${cardMarkup(c,label(team))}`).join("")}</div>`;
}
function miniCard(t){
  return `<div class="mini-card"><small>${escapeHtml(label(t))}</small>${activeCards(t).map(c=>`<strong>${escapeHtml(c[2])} ${escapeHtml(c[0])}</strong>`).join("")}</div>`;
}
function activeEffectsMarkup(team){
  const cards=activeCards(team);
  if(!cards.length)return `<div class="callout">Waiting for their card draw.</div>`;
  return `<div class="active-effects"><div class="kicker">ACTIVE CHAOS</div>${cards.map(c=>`<div class="effect-row"><img src="${cardImage(c)}" alt=""><div><strong>${escapeHtml(c[2])} ${escapeHtml(c[0])}</strong><small>${escapeHtml(c[3])}</small></div></div>`).join("")}</div>`;
}
...
function feedbackForm(type){
  ...
  return chrome(`<section class="card"><div class="kicker">${isBug?"BUG REPORT":"FEATURE SUGGESTION"}</div><h2>${isBug?"Tell us what went wrong":"Share your idea"}</h2>
    <div class="form-grid">
      ...
      <div class="callout wide">Automatically included: v7.0 RC1.2, current screen, game mode, tournament code, browser and device.</div>
...
function releaseNotes(){return chrome(`<section class="card"><div class="kicker">WHAT'S NEW</div><h2>v7.0 RC1.2 – The Mayhem Update</h2><div class="list">
  ${["Commercial modular engine","60 illustrated, one-hole-only Chaos Cards","Premium illustrated card fronts","Animated character screen backgrounds","Demo Mode","Shared and live Singles","2-Man and 4-Man Ambrose","Beta Hub and bug reporting","Automatic score/card awards","Designed & Created by Dennis Moran"].map(x=>`<div class="row">✓ ${x}</div>`).join("")}
...
function diagnostics(){
  ...
  return chrome(`<section class="card"><div class="kicker">DEVELOPER DIAGNOSTICS</div><h2>Build information</h2><div class="list">
    <div class="row"><strong>Version</strong><small>v7.0 RC1.2</small></div>
    ...
    <div class="row"><strong>Cache</strong><small>bogey-banter-v7-rc1-2-mayhem</small></div>
...
function about(){return chrome(`<section class="card about-card"><div class="kicker">ABOUT</div><h1>BOGEY BANTER</h1><p>The ultimate social golf tournament app.</p><div class="credits"><span>Designed & Created by</span><strong>Dennis Moran</strong><span>Built with</span><strong>ChatGPT</strong></div><p class="muted">Version 7.0 RC1.2<br>© Bogey Banter</p><button data-nav="more" class="secondary">Back</button></section>`)}
function awardWinners(){
  const totals={};
  for(const team of state.teams){
    for(const [award,playerId] of Object.entries(team.votes||{})){
      totals[award]??={};
      totals[award][playerId]=(totals[award][playerId]||0)+1;
    }
  }
  const players=allPlayers();
  return AWARDS.map(award=>{
    const entries=Object.entries(totals[award]||{});
    if(!entries.length)return [award,[]];
    const max=Math.max(...entries.map(([,count])=>count));
    return [award,entries.filter(([,count])=>count===max).map(([id])=>players.find(p=>p.id===id)).filter(Boolean)];
  });
}
function awards(){
  if(state.event.status!=="finished"){
    return chrome(`<section class="card"><div class="kicker">AWARDS</div><h2>Voting opens when the final hole is complete.</h2></section>`);
  }
  const voter=votingTeam();
  if(!allVotesSubmitted()&&voter&&!voter.votesLocked){
    const players=allPlayers();
    return chrome(`<section class="card"><div class="kicker">FUN AWARDS VOTING</div><h2>${escapeHtml(label(voter))}'s votes</h2>
      <p class="muted">Choose one individual for every category. Your own player or teammates stay visible but are greyed out.</p>
      <div class="award-vote-list">${AWARDS.map((award,index)=>`<fieldset class="award-vote"><legend>${escapeHtml(award)}</legend>
        ${players.map(p=>{
          const blocked=p.teamSlot===voter.slot;
          const reason=state.event.gameMode==="singles"?"You":"Your Team";
          return `<label class="player-vote ${blocked?"ineligible":""}"><input type="radio" name="award-${index}" value="${escapeHtml(p.id)}" ${blocked?"disabled":""}><span class="player-avatar">${escapeHtml(p.name.slice(0,1).toUpperCase())}</span><span><strong>${escapeHtml(p.name)}</strong><small>${blocked?reason:escapeHtml(p.teamName)}</small></span></label>`;
        }).join("")}</fieldset>`).join("")}</div>
      <button id="submitVotes" class="primary">Submit All Votes</button></section>`);
  }
  if(!allVotesSubmitted()){
    return chrome(`<section class="card"><div class="kicker">VOTING PROGRESS</div><h2>Waiting for remaining votes</h2><div class="list">${state.teams.map(t=>`<div class="row"><div class="row-head"><strong>${escapeHtml(label(t))}</strong><span class="badge ${t.votesLocked?"ready":"waiting"}">${t.votesLocked?"Complete":"Waiting"}</span></div></div>`).join("")}</div></section>`);
  }
  const automatic=autoAwardData();
  const voted=awardWinners();
  return chrome(`<section class="card awards-ceremony"><div class="kicker">AWARDS CEREMONY</div><h1>Round Complete</h1>
    <div class="list">${voted.map(([award,winners])=>`<div class="award-winner"><span>🏆</span><div><strong>${escapeHtml(award)}</strong><p>${winners.map(w=>`${escapeHtml(w.name)} <small>${escapeHtml(w.teamName)}</small>`).join(" & ")||"No votes"}</p></div></div>`).join("")}</div>
  </section><section class="card"><div class="kicker">AUTOMATIC RESULTS</div><div class="list">${automatic.map(([a,t])=>`<div class="row"><div class="row-head"><strong>${escapeHtml(a)}</strong><span>${escapeHtml(label(t))}</span></div></div>`).join("")}</div></section>`);
}
...
function adminBanner(){
  if(!state.event||state.event.status!=="playing"||!state.isAdmin||isShared()||!allSubmitted())return"";
...
function screen(){
  if(state.screen==="about"&&state.mode==="play")return about();
...
function beerBreakOverlay(){
  if(state.event?.status!=="playing"||!state.event?.beerStops||!state.event?.breakHole||state.event.breakHole!==state.event.hole)return"";
  const t=isShared()?state.teams[0]:state.teams.find(x=>x.slot===state.teamSlot);
  if(t?.readyBreakHole===state.event.hole)return"";
  return `<section class="event-overlay beer-break-overlay"><div class="event-panel"><img src="./assets/backgrounds/beer.jpg" alt=""><div class="event-copy"><div class="event-icon">🍺</div><h1>BEER & BUMP!</h1><h2>Hole ${state.event.hole}</h2><p>Grab a drink, have a laugh and get ready for the next hole.</p><button id="continueBeerBreak" class="primary">Continue</button></div></div></section>`;
}
function revealOverlay(){
  if(!revealState?.cards?.length)return"";
  const card=revealState.cards[revealState.index||0];
  return `<section class="event-overlay card-reveal-overlay"><div class="flip-stage"><div class="flip-card"><div class="flip-card-back"><span>BOGEY<br>BANTER</span></div><div class="flip-card-front"><img src="${cardImage(card)}" alt="${escapeHtml(card[0])}"></div></div></div><div class="reveal-caption"><span>${escapeHtml(card[1])} • CARD ${String(card[6]||0).padStart(3,"0")}</span><h1>${escapeHtml(card[0])}</h1><p>${escapeHtml(card[3])}</p><button id="continueCardReveal" class="primary">${(revealState.index||0)<revealState.cards.length-1?"Reveal Next Card":"Continue"}</button></div></section>`;
}
function render(){
  const base=(!splashDone&&!new URLSearchParams(location.search).get("event"))?splash()+screen():screen();
  app.innerHTML=base+beerBreakOverlay()+revealOverlay();
  bind();
...
function bind(){
  ...
  $("#drawBonusCards")?.addEventListener("click",drawBonusCards);
  $("#continueCardReveal")?.addEventListener("click",continueCardReveal);
  $("#continueBeerBreak")?.addEventListener("click",continueBeerBreak);
  $("#submitVotes")?.addEventListener("click",submitVotes);
...
async function saveRegistration(){
  ...
  if(state.event.gameMode!=="singles"&&!name)return toast("Choose a team name.","error");
  if(state.teams.some(x=>x.slot!==t.slot&&x.name===name))return toast("That team name has already been selected.","error");
  await updateTeam(t.slot,{name,players,ownerUid:state.user.uid});toast("Entry saved.");
}
function randomCard({excludeCombo=false,excludeNumbers=[]}={}){
  const pool=CHAOS_CARDS.filter(c=>(!excludeCombo||!isChaosCombo(c))&&!excludeNumbers.includes(c[6]));
...
async function drawNow(){
  ...
  if(isDemo()){
    demo.updateDemoTeam(t.slot,{drawnHole:state.event.hole,card:c,bonusCards:[],categoryCounts:counts});
    demo.updateDemoEvent({sharedRevealSlot:isShared()?t.slot:null,story:[...(state.event.story||[]),`Hole ${state.event.hole}: ${label(t)} drew ${c[0]}.`]});
  }else{
    await updateTeam(t.slot,{drawnHole:state.event.hole,card:c,bonusCards:[],categoryCounts:counts});
    await updateEvent({story:[...(state.event.story||[]),`Hole ${state.event.hole}: ${label(t)} drew ${c[0]}.`],...(isShared()?{sharedRevealSlot:t.slot}:{})});
  }
  revealState={cards:[c],index:0};
  render();
}
async function drawBonusCards(){
  const t=ownTeam();if(!t)return;
  const count=bonusRequired(t);
  const picked=[];
  while(picked.length<count){
    const c=randomCard({excludeCombo:true,excludeNumbers:[t.card?.[6],...picked.map(x=>x[6])]});
    picked.push(c);
  }
  const counts={...(t.categoryCounts||{})};
  picked.forEach(c=>counts[c[1]]=(counts[c[1]]||0)+1);
  await updateTeam(t.slot,{bonusCards:picked,categoryCounts:counts});
  await updateEvent({story:[...(state.event.story||[]),`${label(t)} activated ${t.card[0]} and drew ${picked.map(c=>c[0]).join(" + ")}.`]});
  revealState={cards:picked,index:0};
  render();
}
function continueCardReveal(){
  if(!revealState)return;
  if((revealState.index||0)<revealState.cards.length-1){revealState.index=(revealState.index||0)+1;render();return}
  revealState=null;render();
}
async function continueBeerBreak(){
  const t=isShared()?state.teams[0]:state.teams.find(x=>x.slot===state.teamSlot);
  if(t)await updateTeam(t.slot,{readyBreakHole:state.event.hole});
}
async function submitVotes(){
  const voter=votingTeam();if(!voter)return;
  const votes={};
  for(let i=0;i<AWARDS.length;i++){
    const selected=document.querySelector(`input[name="award-${i}"]:checked`);
    if(!selected)return toast("Choose one eligible player for every award.","error");
    const player=allPlayers().find(p=>p.id===selected.value);
    if(!player||player.teamSlot===voter.slot)return toast("You cannot vote for yourself or anyone on your team.","error");
    votes[AWARDS[i]]=selected.value;
  }
  await updateTeam(voter.slot,{votes,votesLocked:true});
  if(isShared()&&voter.slot<state.teams.length)await updateEvent({currentVoteSlot:voter.slot+1});
  toast("Votes locked in.");
}
...
async function nextHole(){
  if(state.event.hole>=state.event.holes){
    await updateEvent({awardsOpen:true,status:"finished",currentVoteSlot:1,story:[...(state.event.story||[]),"Round complete. Fun awards voting opened."]});
    navigate({screen:"awards"});return;
  }
  const n=state.event.hole+1;
  const beerBreak=Boolean(state.event.beerStops&&n%3===0);
  await updateEvent({hole:n,status:"playing",breakHole:beerBreak?n:null,currentTurnSlot:1,currentScoreSlot:1,sharedRevealSlot:null,sharedPhase:isShared()?"drawing":null,story:[...(state.event.story||[]),...(beerBreak?[`Beer & Bump break opened for Hole ${n}.`]:[]),`Hole ${n} unlocked.`]});
...
async function sendFeedback(){
  ...
  const payload={type,title,body,expected,version:"7.0 RC1.2",screen:state.screen,gameMode:state.event?.gameMode||null,playStyle:state.event?.playStyle||null,eventCode:state.eventCode||null,userAgent:navigator.userAgent,platform:navigator.platform||"unknown",viewport:`${innerWidth}x${innerHeight}`};
