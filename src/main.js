...
function chrome(content,nav=true){
  ...
  return `<main class="shell screen-art screen-art-${state.screen||state.mode}">
    <header class="topbar">
      ...
      ${isDemo()?'<span class="demo-ribbon">DEMO MODE</span>':'<span class="version">v7.0 RC1</span>'}
    </header>
...
function cardMarkup(t){const c=t.card;return `<div class="chaos-card chaos-${cardClass(c)}" data-art="${escapeHtml(c?.[4]||"generic")}"><div class="card-art-layer" aria-hidden="true"><img src="./assets/splash-characters-approved.jpeg" alt=""></div><div class="card-content"><div class="rarity">${escapeHtml(c[1])} • HOLE ONLY</div><div class="chaos-title"><span>${escapeHtml(c[2])}</span><h2>${escapeHtml(c[0])}</h2></div><div class="rule-panel"><p>${escapeHtml(c[3])}</p></div></div><div class="card-footer"><span>${escapeHtml(label(t))}</span><span>Hole ${state.event.hole}</span></div></div>`}
...
function feedbackForm(type){
  ...
  return chrome(`<section class="card"><div class="kicker">${isBug?"BUG REPORT":"FEATURE SUGGESTION"}</div><h2>${isBug?"Tell us what went wrong":"Share your idea"}</h2>
    <div class="form-grid">
      ...
      <div class="callout wide">Automatically included: v7.0 RC1, current screen, game mode, tournament code, browser and device.</div>
...
function releaseNotes(){return chrome(`<section class="card"><div class="kicker">WHAT'S NEW</div><h2>v7.0 RC1</h2><div class="list">
...
function diagnostics(){
  ...
  return chrome(`<section class="card"><div class="kicker">DEVELOPER DIAGNOSTICS</div><h2>Build information</h2><div class="list">
    <div class="row"><strong>Version</strong><small>v7.0 RC1</small></div>
    ...
    <div class="row"><strong>Cache</strong><small>bogey-banter-v7-rc1</small></div>
...
function bind(){
  ...
  document.querySelectorAll("[data-more]").forEach(b=>b.addEventListener("click",()=>setState({screen:b.dataset.more})));
...
async function sendFeedback(){
  ...
  try{if(isDemo()){localStorage.setItem(`bb-feedback-${Date.now()}`,JSON.stringify(payload))}else await live.submitFeedback(payload);toast("Thanks — your report was submitted.");setState({screen:"more"})}catch(e){console.error(e);localStorage.setItem(`bb-feedback-${Date.now()}`,JSON.stringify(payload));toast("Saved on this device. It can be submitted when connected.")}
}
